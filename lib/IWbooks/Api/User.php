...
function iw_books_userapi_reble($args)
{
	...
	$output = '';
	$all = ModUtil::getVar('iw_books', 'nivells');
	$all = explode('|', $all);
	foreach ($all as $item){
		$level = explode('#',$item);		
...
function iw_books_userapi_getall_mat($args)
{
	...
	for (; !$result->EOF; $result->MoveNext()) {
		...
		if (SecurityUtil::checkPermission*(0, 'iw_books::', "$autor::$tid", ACCESS_READ)) {
			$items[] = array('tid'       => $tid,
                             'codi_mat'  => $codi_mat,
                             'materia'   => $materia);
		}
...
function iw_books_userapi_plans($args)
{
	...
	$plans = explode("|",ModUtil::getVar('iw_books', 'plans') );
...
function iw_books_userapi_nivellsselec($args)
{
	...
	$nivells = explode("|",ModUtil::getVar('iw_books', 'nivells') );
...
*//*
	...
	$darrer_nivell = ModUtil::getVar('iw_books', 'darrer_nivell');
	
	for ($i=1; $i<=$darrer_nivell ; $i++){
		$data[$i] = ModUtil::apiFunc('iw_books', 'user', 'reble', array('nivell' => $i)) ;
	}
...
function iw_books_userapi_anys($args)
{
	...
	for (; !$result->EOF; $result->MoveNext()) {
		...
		$cursacad = ModUtil::apiFunc('iw_books', 'user', 'cursacad', array('any' => $anytria));
