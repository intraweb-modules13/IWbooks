...
class IWbooks_Controller_User extends Zikula_AbstractController {
    ...
    public function main() {
        ...
        return $view->fetch('IWbooks_user_main.htm');
    }
    ...
    public function view() {
        ...
        if (FormUtil::getPassedValue('pdf') != '') {
            ...
            /*
              $file = ModUtil::apiFunc('IWbooks', 'user', 'generapdf', array('any' => $any,
              'materia' => $materia,
              'etapa' => $etapa,
              'nivell' => $nivell));
             */
     ...
     */
    function IWbooks_usermenu() {
        $output = & new pnHTML();

        $cursacademic = ModUtil::getVar('IWbooks', 'any');
        ...
        $output->FormStart(ModUtil::url('IWbooks', 'user', 'view'));

        $output->LineBreak();
        $output->TableRowStart();
        $output->TableColStart();
        $output->Text($this->__('Try course:'));
        $output->FormText('any', $cursacademic, 4, 4);
        $output->TableColEnd();

        $data = ModUtil::apiFunc('IWbooks', 'user', 'plans');
        $output->TableColStart();
        $output->Text($this->__('Plan:'));
        $output->FormSelectMultiple('etapa', $data);
        $output->TableColEnd();

        $data2 = ModUtil::apiFunc('IWbooks', 'user', 'nivells');
        $output->TableColStart();
        $output->Text($this->__('Level:'));
        $output->FormSelectMultiple('nivell', $data2);
        $output->TableColEnd();
        $output->TableRowEnd();

        $data3 = ModUtil::apiFunc('IWbooks', 'user', 'materies');
        $output->TableRowStart();
        $output->TableColStart();
        $output->Text($this->__('Subject:'));
        $output->FormSelectMultiple('materia', $data3);
        $output->TableColEnd();

        $output->TableColStart();
        $output->Text($this->__('Reads:'));
        $output->FormCheckbox('lectura', $checked = false, $value = '1');
        $output->TableColEnd();
        $output->TableRowStart();
        $output->TableColStart();
        $output->Linebreak(2);
        $output->FormSubmit($this->__('See list of books'));
        $output->TableColEnd();

        $output->TableColStart();
        $output->Linebreak(2);
        $output->Text('<input type="submit" value="' . $this->__('Creating listed on page PDF') . '" name="pdf" tabindex="7" />');
        $output->TableColEnd();
        $output->TableRowEnd();
    ...
    function IWbooks_usermenu1() {
        ...
        $output->TableEnd();
        $output->Linebreak(1);
        $output->URL(DataUtil::formatForDisplay(ModUtil::url('IWbooks', 'user', 'main')), $this->__('Back'));
        $output->Linebreak(2);
